...
class SingleModelRouter : public ModelRouter {
 public:
  ...
  bool RegisterModel(const ModelInfo& info, std::shared_ptr<LlamaCPUBackend> backend);
  ...
  std::shared_ptr<LlamaCPUBackend> GetBackend(const std::string& model_id) override;
