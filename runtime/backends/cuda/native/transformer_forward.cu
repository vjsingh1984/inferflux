...
template <typename T>
LlamaForwardTyped<T>::~LlamaForwardTyped() {
  FreeScratchBuffers();
}

template <typename T>
bool LlamaForwardTyped<T>::AllocateScratch() {
  ...
  size_t rows = static_cast<size_t>(
      std::max(max_seq_len_, max_batch_size_));

  if (!alloc(&d_hidden_, rows * hidden_size_)) return false;
  if (!alloc(&d_residual_, rows * hidden_size_)) return false;
  if (!alloc(&d_norm_out_, rows * hidden_size_)) return false;
  if (!alloc(&d_q_, rows * num_heads_ * head_dim_)) return false;
  if (!alloc(&d_k_new_, rows * num_kv_heads_ * head_dim_)) return false;
  if (!alloc(&d_v_new_, rows * num_kv_heads_ * head_dim_)) return false;
  if (!alloc(&d_attn_out_, rows * num_heads_ * head_dim_)) return false;
  if (!alloc(&d_ffn_gate_, rows * intermediate_size_)) return false;
  if (!alloc(&d_ffn_up_, rows * intermediate_size_)) return false;
  if (!alloc(&d_ffn_down_, rows * hidden_size_)) return false;
  ...
  cudaError_t err =
      cudaMalloc(&d_token_ids_, rows * sizeof(int));
...
template <typename T>
void LlamaForwardTyped<T>::FreeScratchBuffers() {
  auto free_buf = [](T** ptr) {
    if (*ptr) {
      cudaFree(*ptr);
      *ptr = nullptr;
...
bool LlamaForwardTyped<T>::Forward(const std::vector<int>& token_ids,
  ...
  if (seq_len > max_seq_len_) {
    log::Error("llama_forward",
               "seq_len " + std::to_string(seq_len) + " exceeds max " +
  ...
  err = cudaMemcpyAsync(d_token_ids_, token_ids.data(),
  ...
  for (int layer = 0; layer < num_layers_; layer++) {
    ...
    // 4b-d: Q/K/V projections + optional bias
    {
      ...
      if (!gemm_->GemmTyped<T>(seq_len, num_kv_heads_ * head_dim_, hidden_size_,
  ...
  // Step 5: Final RMSNorm (last token only)
  {
    ...
    if (!gemm_->GemmTyped<T>(1, vocab_size_, hidden_size_, d_norm_out_, lm_head,
                              d_logits_typed_)) {
...
bool LlamaForwardTyped<T>::BatchForward(const std::vector<int>& token_ids,
  ...
  // Step 5: Final RMSNorm + LM head for each sequence
  {
    ...
    // Batched LM head: [B, hidden_size] x [vocab_size, hidden_size]^T -> [B, vocab_size]
    const T* lm_head = reinterpret_cast<const T*>(weights_->LmHead());
