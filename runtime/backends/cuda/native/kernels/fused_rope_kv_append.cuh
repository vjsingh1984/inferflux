...
// ============================================================================
// FusedRoPEKvAppendIndirect: Same as Strided but uses slot base pointer table
// instead of buffer + seq_id * slot_stride. Supports hybrid KV cache.
// ============================================================================

template <typename T>
__global__ void FusedRoPEKvAppendIndirectKernel(
    T *__restrict__ q,
    T *__restrict__ k_new,
    const T *__restrict__ v_new,
    T *const *__restrict__ slot_base_ptrs,
    const int *__restrict__ d_seq_ids,
    const int *__restrict__ d_n_past,
    int layer, int batch_size, int num_heads, int num_kv_heads, int head_dim,
    int kv_dim, size_t layer_stride, size_t kv_stride,
    float freq_base, int rope_type) {
  const int idx = blockIdx.x * blockDim.x + threadIdx.x;
  const int half_dim = head_dim / 2;
  const int q_pairs_per_seq = num_heads * half_dim;
  ...
  if (local < q_pairs_per_seq) {
    ...
    // Scatter-write rotated K to KV cache via indirect pointer
    const size_t layer_offset = static_cast<size_t>(layer) * layer_stride;
    const size_t base_cache =
        layer_offset + static_cast<size_t>(d_n_past[b]) * kv_dim;
    ...
    slot[base_cache + static_cast<size_t>(elem0)] = rotated0;
    slot[base_cache + static_cast<size_t>(elem1)] = rotated1;
    ...
    T *slot = slot_base_ptrs[d_seq_ids[b]];
    slot[v_offset] = v_new[b * kv_dim + d];
  }
}

template <typename T>
cudaError_t FusedRoPEKvAppendIndirect(
    T *q, T *k_new, const T *v_new, T *const *slot_base_ptrs,
    const int *d_seq_ids, const int *d_n_past, int layer, int batch_size,
    int num_heads, int num_kv_heads, int head_dim, int kv_dim,
    size_t layer_stride, size_t kv_stride, float freq_base,
  ...
  FusedRoPEKvAppendIndirectKernel<T><<<blocks, threads, 0, stream>>>(
      q, k_new, v_new, slot_base_ptrs, d_seq_ids, d_n_past, layer, batch_size,
      num_heads, num_kv_heads, head_dim, kv_dim, layer_stride, kv_stride,
      freq_base, rope_type);
  return cudaGetLastError();
}
