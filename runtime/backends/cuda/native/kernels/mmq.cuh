...
// ============================================================================
// MMQ accumulate kernels (read-modify-write epilogue for residual fusion)
//
// Identical to the non-accumulate variants except the writeback adds to the
// existing output: output[i] += result[i]. This eliminates a separate
// ResidualAdd kernel launch for O-proj and down-proj at batch sizes 9-64.
// ============================================================================

template <int kTileM>
__global__ void inferflux_mmq_q4k_accum(const block_q4_k *__restrict__ weight,
                                        const block_q8_1 *__restrict__ act_q8_1,
                                        half *__restrict__ output, int M, int N,
                                        int K) {
  constexpr int kQ8PerSB = 8;
...
#pragma unroll
      for (int r = 0; r < kTileM; ++r) {
        if (r < num_rows) {
          output[(row_base + r) * N + out_idx] =
              __float2half(acc[r] + __half2float(output[(row_base + r) * N + out_idx]));
        }
      }
    }
  }
...
template <int kTileM>
__global__ void inferflux_mmq_q6k_accum(const block_q6_k *__restrict__ weight,
                                        const block_q8_1 *__restrict__ act_q8_1,
                                        half *__restrict__ output, int M, int N,
                                        int K) {
  constexpr int kQ8PerSB = 8;
...
template <int kTileM>
__global__ void inferflux_mmq_q8_0_accum(const block_q8_0 *__restrict__ weight,
                                         const block_q8_1 *__restrict__ act_q8_1,
                                         half *__restrict__ output, int M, int N,
                                         int K) {
  constexpr int kBlocksPerIter = 4;
...
template <int kTileM>
__global__ void inferflux_mmq_q8k_accum(const block_q8_k *__restrict__ weight,
                                        const block_q8_1 *__restrict__ act_q8_1,
                                        half *__restrict__ output, int M, int N,
                                        int K) {
  constexpr int kQ8PerSB = 8;
...
// Accumulate variant: output[i] += result[i] (for residual fusion)
template <typename BlockT,
          void (*Kernel16)(const BlockT *, const block_q8_1 *, half *, int, int,
                           int),
          void (*Kernel32)(const BlockT *, const block_q8_1 *, half *, int, int,
                           int)>
bool DispatchMmqAccum(const void *data, const void *act_q8_1, half *output,
                      int M, int N, int K, int q8_per_sb,
                      cudaStream_t stream) {
  auto *w = static_cast<const BlockT *>(data);
  auto *a = static_cast<const block_q8_1 *>(act_q8_1);
