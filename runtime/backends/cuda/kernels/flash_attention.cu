...
// ============================================================================
// Batched FlashDecode kernel (decode-only: query_len=1 per sequence)
//
// Grid: (num_heads, batch_size)
// Each block handles one (head, sequence) pair. Reads K/V pointers and kv_len
// from device arrays. Reuses the same tiled FA2 loop with online softmax.
// ============================================================================

template <typename T>
__global__ void
FlashDecodeTypedKernel(const T *__restrict__ Q, T *__restrict__ O,
                       const T *const *__restrict__ d_K_ptrs,
                       const T *const *__restrict__ d_V_ptrs,
                       const int *__restrict__ d_kv_lens, int num_heads,
  ...
  const int kv_len = d_kv_lens[batch_idx];
  const int kv_dim = num_kv_heads * head_dim;

  // Load Q register for this thread's dimension
  float q_reg = 0.0f;
  if (d < head_dim) {
    size_t q_offset =
        (size_t)batch_idx * num_heads * head_dim + head_idx * head_dim + d;
    q_reg = DtypeTraits<T>::to_float(Q[q_offset]);
  }

  // K/V pointers for this sequence
  const T *K = d_K_ptrs[batch_idx];
  const T *V = d_V_ptrs[batch_idx];
  ...
  for (int kv_start = 0; kv_start < kv_len; kv_start += FA2_TILE_KV) {
    ...
    for (int i = d; i < total_elements; i += num_threads) {
      ...
      int dim = i % head_dim;
      size_t kv_offset =
          (size_t)(kv_start + t) * kv_dim + kv_head_idx * head_dim + dim;
      s_k[i] = DtypeTraits<T>::to_float(K[kv_offset]);
      s_v[i] = DtypeTraits<T>::to_float(V[kv_offset]);
    }
    __syncthreads();

    // Warp-level dot products
    for (int t = 0; t < tile_len; t++) {
      float partial = (d < head_dim) ? q_reg * s_k[t * head_dim + d] : 0.0f;
#pragma unroll
      for (int offset = 16; offset > 0; offset >>= 1)
...
cudaError_t FlashDecodeTyped(const T *Q, T *O, const T *const *d_K_ptrs,
  ...
  FlashDecodeTypedKernel<T><<<grid, threads, smem, stream>>>(
      Q, O, d_K_ptrs, d_V_ptrs, d_kv_lens, num_heads, num_kv_heads, head_dim,
      scale);
  return cudaGetLastError();
}
...
template cudaError_t FlashDecodeTyped<half>(const half *, half *,
                                            const half *const *,
                                            const half *const *, const int *,
                                            int, int, int, int, float,
                                            cudaStream_t);
template cudaError_t
FlashDecodeTyped<__nv_bfloat16>(const __nv_bfloat16 *, __nv_bfloat16 *,
                                const __nv_bfloat16 *const *,
                                const __nv_bfloat16 *const *, const int *, int,
