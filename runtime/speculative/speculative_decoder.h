...
public:
  SpeculativeDecoder(SpeculativeConfig config,
                     ...
                     std::shared_ptr<LlamaCppBackend> draft_backend);
  ...
  Validate(const std::vector<int> &prompt_tokens, const SpeculativeDraft &draft,
           int max_new_tokens, std::shared_ptr<LlamaCppBackend> target_backend);
...
private:
  ...
  std::shared_ptr<LlamaCppBackend> draft_backend_;
