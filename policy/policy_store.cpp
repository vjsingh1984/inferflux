...
void PolicyStore::SetApiKey(const std::string &key,
...
void PolicyStore::SetGuardrailBlocklist(
...
bool PolicyStore::Decrypt(const std::string &encrypted,
  ...
  auto parseLine = [&](const std::string &expected_prefix,
  ...
  if (!parseLine("nonce", &nonce))
    return false;
  if (!parseLine("tag", &tag))
    return false;
  if (!parseLine("data", &data))
    return false;
