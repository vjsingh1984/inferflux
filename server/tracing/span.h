...
// Format:  traceparent: 00-<trace-id(32 hex)>-<parent-id(16 hex)>-<flags(2
// hex)>
...
inline SpanContext ChildContext(const SpanContext &parent) {
  std::string tid =
      parent.trace_id.size() == 32 ? parent.trace_id : RandomHex(16);
...
public:
  using OnEnd =
      std::function<void(const std::string &, const SpanContext &, double)>;

  Span(std::string name, SpanContext context, OnEnd on_end = nullptr)
      : name_(std::move(name)), context_(std::move(context)),
        on_end_(std::move(on_end)), start_(std::chrono::steady_clock::now()) {}
