...
public:
  ...
  // General-purpose structured log entry (subject, model, status, metadata
  // message).
  void Log(const std::string &subject, const std::string &model,
           const std::string &status, const std::string &message);

  // Log a generation request/response pair. In production mode
  // (debug_mode=false), prompt and response are SHA-256 hashed before writing
  // to satisfy the Architecture.md §Security requirement for prompt redaction.
  void LogRequest(const std::string &subject, const std::string &model,
