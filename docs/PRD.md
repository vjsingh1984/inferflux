...
- **Pool Health CLI (Done)**: `inferctl admin pools --get` now reports readiness role/state (`/readyz`) plus scheduler queue depth and batch-limit gauges parsed from `/metrics`, giving operators a first-class pool-health snapshot for disaggregated deployments.
...
- **Phased Prefill/Decode** (§2.5): `LlamaCPUBackend` gains `Prefill()`/`Decode()`/`FreeSequence()`/`SerializeSequence()`/`HydrateSequence()`/`CopySequencePrefix()`/`PrefillPartial()`. `Scheduler` allocates slots from a `seq_slots_free_` free-list; `DecodeWorkerLoop` drains `pending_decode_` in a dedicated thread pool when `decode_pool_size > 0`; `BuildBatchLocked` skips decode queue when workers are active. KVChannel gated on `use_decode_workers_` to prevent fill-deadlock. KV warm prefix store (4-slot LRU, `weak_ptr` backends, BPE-correct `n_kv_tokens`, eviction-safety guard, expired-slot purge) allows requests sharing a prompt prefix to skip re-evaluating those tokens.
