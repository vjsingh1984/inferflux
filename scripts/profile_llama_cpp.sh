#!/usr/bin/env bash
#
# cuda_llama_cpp (llama.cpp) Nsight Systems Profiling Script
#
# Profiles cuda_llama_cpp execution at different concurrency levels
# to understand why it scales 2.59x from c=1 to c=16 vs cuda_native's 1.11x.
#
# Usage:
#   ./scripts/profile_llama_cpp.sh [model.gguf]
#   CONCURRENCY="1,16" ./scripts/profile_llama_cpp.sh model.gguf
...
OUTPUT_DIR="./llama_cpp_profile_$(date +%Y%m%d_%H%M%S)"
API_KEY="dev-key-123"
PORT=18091
...
header "cuda_llama_cpp Nsight Systems Profiling"
echo "Model: $MODEL_PATH"
echo "Backend: cuda_llama_cpp (llama.cpp)"
...
# Generate server config with cuda_llama_cpp backend
...
runtime:
  ...
  cuda:
    ...
    phase_overlap:
      enabled: true
  backend_exposure:
    prefer_native: false  # Use llama.cpp backend
...
profile_concurrency() {
    ...
    header "Profiling cuda_llama_cpp at concurrency=$concurrency"

    # Start server in background with profiling
    log "Starting cuda_llama_cpp server with profiling..."
