...
echo ============================================================
echo  A/B Throughput Comparison: CUDA Graphs ON vs OFF
echo  Model: Qwen2.5-3B Q4_K_M, 100 tokens, RTX 4000 Ada
echo ============================================================

echo.
echo --- Run 1: CUDA Graphs OFF ---
set INFERFLUX_DISABLE_CUDA_GRAPH=1
%PROBE% --backend inferflux_cuda --model %MODEL% --prompt "The capital of France is" --top-n 5 --max-tokens 100 2>&1 | findstr /R "token_count ok native_sync_trace.*batch_result_ready CUDA.graph"

echo.
echo --- Run 2: CUDA Graphs ON ---
set INFERFLUX_DISABLE_CUDA_GRAPH=
%PROBE% --backend inferflux_cuda --model %MODEL% --prompt "The capital of France is" --top-n 5 --max-tokens 100 2>&1 | findstr /R "token_count ok native_sync_trace.*batch_result_ready CUDA.graph"

echo.
echo --- Run 3: CUDA Graphs OFF (confirm) ---
set INFERFLUX_DISABLE_CUDA_GRAPH=1
%PROBE% --backend inferflux_cuda --model %MODEL% --prompt "The capital of France is" --top-n 5 --max-tokens 100 2>&1 | findstr /R "token_count ok native_sync_trace.*batch_result_ready CUDA.graph"

echo.
echo --- Run 4: CUDA Graphs ON (confirm) ---
set INFERFLUX_DISABLE_CUDA_GRAPH=
%PROBE% --backend inferflux_cuda --model %MODEL% --prompt "The capital of France is" --top-n 5 --max-tokens 100 2>&1 | findstr /R "token_count ok native_sync_trace.*batch_result_ready CUDA.graph"
