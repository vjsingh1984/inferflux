...
std::string BuildRequest(const ParsedUrl& parsed,
...
HttpClient::RawConnection HttpClient::SendRaw(const std::string& method,
  ...
  if (parsed.use_tls) {
    ...
    while (send_remaining > 0) {
      int sent = SSL_write(conn.ssl, send_ptr, static_cast<int>(send_remaining));
      if (sent <= 0) {
...
ssize_t HttpClient::RecvRaw(RawConnection& conn, char* buffer, std::size_t length) const {
  if (conn.ssl) {
    while (true) {
